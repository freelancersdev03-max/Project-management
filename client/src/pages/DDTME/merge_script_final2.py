...
# 5. Extract Modal Code accurately
modal_start = ddtme.find('{showUploadModal && (')
# find the closing `)}`
stack = 0
modal_end = -1
for i in range(modal_start + 21, len(ddtme)):
    if ddtme[i] == '(':
        stack += 1
    elif ddtme[i] == ')':
        if stack == 0:
...
if modal_end != -1:
    modal_code = "{/* ---- Upload Excel Column Mapping Modal ---- */}\n      " + ddtme[modal_start:modal_end+1] + "}\n"
else:
    modal_code = ""

# 6. Extract UI from Demo
demo_return_idx = demo.find('  return (')
demo_ui = demo[demo_return_idx:]

# Adapt Demo UI to real state
...
# Inject the Modal right before the final closing divs!
# CapacityPlannerDemo.jsx ends with:
#       </div>
#     </div>
#   );
# }
# So we inject it before `      </div>\n    </div>\n  );`

...
export default function DDFMSBasePage() {
  ...
  const getProfileRoute = () => {
    ...
    if (role === 'KAYAARA' || role === 'MLS') return '/kayaara';
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                {filteredClients.map((client, index) => (
                  <DDFMSClientCard key={client.id} data={client} index={index} />
                ))}
              </div>
            ) : (
...
const DDFMSClientCard = ({ data, index = 0 }) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mt-8 relative z-10 gap-3">
        ...
        >
          <span className="text-[10px] font-bold uppercase tracking-widest">Open DDFMS</span>

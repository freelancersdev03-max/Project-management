...
# 4. We also need totalHours and activeResources logic injected right before return
metrics_logic = """
  // --- DERIVED METRICS FOR NEW UI ---
...
# Fix Submit button
demo_ui = demo_ui.replace('<button className="flex items-center gap-2 px-5 py-2 bg-slate-900', '<button onClick={handleSendForApproval} disabled={isSubmitting || !canSubmit} className={`flex items-center gap-2 px-5 py-2 bg-slate-900 ${(!canSubmit || isSubmitting) ? "opacity-50 cursor-not-allowed" : ""} text-white text-sm font-semibold rounded-lg hover:bg-slate-800 transition-colors shadow-md`}>\n              <Check size={16} />\n              Submit Approval\n            </button>')

# Fix Download button
demo_ui = demo_ui.replace('<button className="flex items-center gap-2 px-4 py-2 bg-white border', '<button onClick={handleDownloadExcel} className="flex items-center gap-2 px-4 py-2 bg-white border')
...
if modal_code_idx != -1:
    # We find the end of the modal code exactly. 
    # The modal is closed by a `</div>\n      )}`. Let's just find `      )}` after the modal start.
...
# Since we appended the modal code inside the max-w container, we just use the original closing logic of the demo UI.
# BUT we need to ensure the final closing is correct for an arrow function, and not just `}`.
# The original DDTMETable ends with `  );\n}\nexport default DDTMETable;` 
# Oh wait, `DDTMETable.jsx` actually ends with `  );\n};\n\nexport default DDTMETable;` or similar? Let's assume `};\nexport default DDTMETable;`
demo_ui = demo_ui.replace('  );\n}\n', '  );\n};\nexport default DDTMETable;\n')

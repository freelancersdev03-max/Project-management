              ...
              <div className="grid grid-cols-2 gap-2.5 mt-1">
                <button
                  ...
                  onClick={async () => {
                    try {
                      setIsSubmitting(true);
                      const res = await api.post('/sso/login/', { provider: 'microsoft' });
                      if (res.data?.auth_url) {
                        window.location.href = res.data.auth_url;
                      }
                    } catch (err) {
                      // Fallback dev login
                      const cb = await api.post('/sso/callback/', { provider: 'microsoft', code: 'mock_ms_user' });
                      if (cb.data?.access) {
                        localStorage.setItem("access_token", cb.data.access);
                        localStorage.setItem("token", cb.data.access);
                        localStorage.setItem("role", cb.data.user.role);
                        setLoginSuccess(true);
                  ...
                  className="flex items-center justify-center gap-2 py-2 px-3 border border-gray-200 rounded-lg text-xs font-medium text-gray-700 bg-white hover:bg-gray-50 hover:border-gray-300 transition-all shadow-sm"
                >
                ...
                <button
                  ...
                  onClick={async () => {
                    try {
                      setIsSubmitting(true);
                      const res = await api.post('/sso/login/', { provider: 'google' });
                      if (res.data?.auth_url) {
                        window.location.href = res.data.auth_url;
                      }
                    } catch (err) {
                      const cb = await api.post('/sso/callback/', { provider: 'google', code: 'mock_google_user' });
                      if (cb.data?.access) {
                        localStorage.setItem("access_token", cb.data.access);
                        localStorage.setItem("token", cb.data.access);
                        localStorage.setItem("role", cb.data.user.role);

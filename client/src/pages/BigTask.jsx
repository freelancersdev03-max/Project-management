...
const BigTask = ({ projectId, onProgressUpdate }) => {
    ...
    const [tasks, setTasks] = useState([
        { id: 1, title: 'Project Initialization & Discovery', startMonth: 0, endMonth: 1, type: 'X', status: 'In Progress', startDate: '2024-01-01', targetDate: '2024-02-15' },
        { id: 2, title: 'Strategic Architecture Design', startMonth: 1, endMonth: 2, type: 'Y', status: 'In Progress', startDate: '2024-02-01', targetDate: '2024-03-30' },
        { id: 3, title: 'Legacy Data Migration', startMonth: 0, endMonth: 0, type: 'X', status: 'Completed', startDate: '2024-01-01', targetDate: '2024-01-31' },
    ]);
    ...
    const getMonthIndex = (dateStr, projectStartStr) => {
        if (!dateStr || !projectStartStr) return 0;
    ...
    const handleAddTask = (e) => {
        ...
        if (project && project.start_date) {
            startMonth = Math.max(0, getMonthIndex(formData.startDate, project.start_date));
            endMonth = Math.max(0, getMonthIndex(formData.targetDate, project.start_date));
        }

        const maxMonthIdx = months.length - 1;
        startMonth = Math.min(startMonth, maxMonthIdx);
        endMonth = Math.min(endMonth, maxMonthIdx);
    ...
    const saveTask = (taskId) => {
        ...
        let endMonth = 0;
        if (project && project.start_date) {
            startMonth = Math.max(0, getMonthIndex(editingStartDate, project.start_date));
            endMonth = Math.max(0, getMonthIndex(editingTargetDate, project.start_date));
        }
    ...
    return (
        <div className="w-full font-sans text-slate-900 bg-white">
                ...
                <table className="w-full border-collapse table-fixed min-w-[800px]">
                    <thead>
                        <tr className="bg-slate-100 divide-x divide-slate-300 border-b border-slate-300">
                            <th className="p-2 w-12 text-center text-[10px] font-bold text-slate-600 uppercase">Sr. No.</th>
                            <th className="p-2 w-1/3 text-left text-[10px] font-bold text-slate-600 uppercase pl-3">Task Description (Dates)</th>
                            <th className="p-2 w-24 text-center text-[10px] font-bold text-slate-600 uppercase">Target Date</th>
                            {months.map((m, i) => (
                                <th key={i} className="p-1 text-center text-[10px] font-bold text-slate-600 uppercase bg-slate-50">{m}</th>
                            ))}
                            <th className="p-2 w-32 text-center text-[10px] font-bold text-slate-600 uppercase">Status</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-300">
                        {processedTasks.map((task, index) => (
                            <tr key={task.id} className={`divide-x divide-slate-300 ${task.type === 'Y' ? 'bg-orange-50' : 'bg-white hover:bg-slate-50'}`}>
                                ...
                                <td className="p-2 text-center text-[10px] font-bold text-slate-600">
                                    {task.targetDate || '-'}
                                </td>

                                {months.map((_, mIdx) => {
                                    const isActive = mIdx >= task.startMonth && mIdx <= task.endMonth;
                                    let cellClass = "p-0 h-12 relative select-none";
                                    if (isActive) {
                                        cellClass += task.status === 'Completed' ? " bg-emerald-500" : " bg-[#3b82f6]";
                                    }
                                    return <td key={mIdx} className={cellClass}></td>;
                                })}
            ...
            {/* --- KPI SECTION --- */}
            <div>
                <div className="flex justify-between items-center mt-8 mb-6 px-1">
                    <div className="flex items-center gap-3">
                        <span className="bg-slate-800 p-1.5 rounded text-white shadow-sm">
                            <Target size={16} />
                        </span>
                        <h1 className="text-xl font-bold tracking-tight text-slate-800 uppercase">KPIs</h1>
                    </div>
                    {canEdit && (
                        <button
                            onClick={() => setIsKpiModalOpen(true)}
                            className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded text-xs font-bold uppercase tracking-wider transition-colors shadow-sm"
                        >

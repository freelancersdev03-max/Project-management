...
import api from '../../api';
import {
  Users, Briefcase, LayoutGrid, CheckCircle2,
  Target, ChevronRight, FileText, CalendarDays,
  Bell, ShieldCheck, Mail, Phone, Clock
} from 'lucide-react';
...
const SeniorProfile = () => {
  ...
  const [userProfile, setUserProfile] = useState({
    ...
    email: "",
    role: "Senior Client Rep",
    photo: null
  });

  const [stats, setStats] = useState({
    supervisedProjects: 6, // Mocked
    teamMembers: 15, // Mocked
    pendingApprovals: 4, // Mocked
    upcomingMeetings: 2 // Mocked
  });
    ...
    const fetchProfile = async () => {
      try {
        ...
        setUserProfile({
          name: displayName,
          email: data.email,
          role: data.role === "SENIOR" ? "Senior Client Rep" : data.role,
          photo: data.photo,
        ...
        const storedRole = localStorage.getItem("role") || "Senior Client Rep";
        if (storedEmail) {
          const namePart = storedEmail.split('@')[0];
          const formattedName = namePart.charAt(0).toUpperCase() + namePart.slice(1);
          setUserProfile(prev => ({ ...prev, name: formattedName, email: storedEmail, role: storedRole }));
        }
      } finally {
        setLoading(false);
      }
  ...
  const kpis = [
    { label: "Supervised Projects", value: stats.supervisedProjects, icon: <Briefcase size={20} />, color: "text-blue-600", bg: "bg-blue-50", trend: "Active" },
    { label: "Team Members", value: stats.teamMembers, icon: <Users size={20} />, color: "text-emerald-600", bg: "bg-emerald-50", trend: "Stable" },
    { label: "Pending Approvals", value: stats.pendingApprovals, icon: <CheckCircle2 size={20} />, color: "text-amber-600", bg: "bg-amber-50", trend: "Action Req" },
    { label: "Upcoming Meetings", value: stats.upcomingMeetings, icon: <CalendarDays size={20} />, color: "text-indigo-600", bg: "bg-indigo-50", trend: "Scheduled" },
    { label: "Reports Generated", value: "14", icon: <FileText size={20} />, color: "text-purple-600", bg: "bg-purple-50", trend: "+2" },
    { label: "Task Efficiency", value: "88%", icon: <Target size={20} />, color: "text-slate-600", bg: "bg-slate-100", trend: "+1.2%" },
  ];
    ...
    <div className="h-screen w-screen bg-[#F8FAFC] antialiased flex overflow-hidden font-sans">
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        <header className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between sticky top-0 z-10">
          <div>
            <h1 className="text-xl font-semibold text-slate-800">Senior Representative Dashboard</h1>
        ...
        <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-6">
            ...
            {kpis.map((kpi, idx) => (
              <div key={idx} className="bg-white border border-slate-200 p-5 rounded-xl shadow-sm hover:shadow-md transition-shadow">
                <div className="flex justify-between items-start">
                  ...
                  <span className={`text-xs font-medium ${kpi.trend.startsWith('+') || kpi.trend === 'Active' || kpi.trend === 'Stable' ? 'text-emerald-600' : kpi.trend === 'Action Req' ? 'text-amber-600' : 'text-slate-500'}`}>
                    {kpi.trend}
                  </span>
                </div>
                <div className="mt-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white border border-slate-200 rounded-xl shadow-sm flex flex-col">
              <div className="px-6 py-5 border-b border-slate-100">
                <h2 className="text-base font-semibold text-slate-800">Senior Identity</h2>
              </div>
              <div className="p-6 flex-1 space-y-6">
                ...
                <div className="space-y-4 pt-4 border-t border-slate-100">
                   ...
                   <div className="flex items-center gap-3 text-sm font-medium text-slate-600">
                      ...
                      <span className="truncate">Tier 2 Access</span>
                   </div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm">
              <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center">
                <h2 className="text-base font-semibold text-slate-800">Pending Actions</h2>
                <span className="bg-amber-100 text-amber-700 text-xs font-bold px-2 py-1 rounded-full">{stats.pendingApprovals}</span>
              </div>
                ...
                {[1, 2].map((item) => (
                  <div key={item} className="p-5 flex items-center justify-between hover:bg-slate-50/50 transition-colors">
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-slate-800">Action Plan Review</p>
                        <p className="text-sm text-slate-500 mt-0.5">Please review the latest task assignments for Project Alpha.</p>
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <button className="px-3 py-1.5 text-xs font-medium bg-white border border-slate-200 text-slate-600 rounded-md hover:bg-slate-50">Review</button>
                    </div>
                  </div>
                ))}
              </div>

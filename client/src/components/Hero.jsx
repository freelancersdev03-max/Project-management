      ...
      <div className="max-w-[1440px] mx-auto px-8 lg:px-12 pt-4 pb-28 grid lg:grid-cols-12 gap-12 items-center">
        ...
        <div className="lg:col-span-5 relative z-10">
          <h1 className="text-5xl md:text-7xl font-extrabold leading-[1.1] text-slate-900">
            Total Business
            <span className="block text-[#F58A4B]">
              Transformation Simplified
            </span>
          </h1>

          <p className="mt-8 max-w-xl text-[25px] text-slate-600 leading-relaxed font-light">
          ...
          <div className="mt-10 flex flex-wrap items-center gap-5">
            ...
            <button
              ...
              className="inline-flex items-center justify-center rounded-full px-10 py-4 text-base font-semibold text-black bg-gradient-to-r from-[#F58A4B] to-orange-200 shadow-lg hover:opacity-90 transition active:scale-95 cursor-pointer"
            >
        ...
        <div className="relative lg:col-span-7 w-full lg:pl-8">
          ...
          <div className="relative group">
            <img
              ...
              className="relative w-full h-auto rounded-[2.5rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.15)] border border-slate-100 transition-all duration-700 lg:scale-110 lg:translate-x-6 group-hover:scale-[1.12]"
            />
          </div>
        </div>
      </div>
          ...
          <div className="relative bg-white w-full max-w-3xl rounded-3xl shadow-2xl overflow-hidden p-8 md:p-12 animate-in fade-in zoom-in duration-500">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {services.map((service, index) => (
                <div key={index} className="p-6 rounded-2xl border border-slate-100 bg-slate-50 hover:border-[#F58A4B]/40 hover:bg-white hover:shadow-xl hover:shadow-orange-500/5 transition-all group">
                  <div className="text-4xl mb-4">{service.icon}</div>
                  <h3 className="text-xl font-bold text-slate-900 mb-2 group-hover:text-[#F58A4B]">{service.title}</h3>
                  <p className="text-slate-600 text-[20px] leading-relaxed">{service.desc}</p>
                </div>
              ))}
            </div>
            <div className="mt-10 text-center">
              <button onClick={() => setIsModalOpen(false)} className="bg-slate-900 text-white px-10 py-3 rounded-full font-semibold hover:bg-black transition-all">Close Window</button>
            </div>
          </div>
        </div>
      )}

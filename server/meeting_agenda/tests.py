...
class MeetingAgendaLogFlowTests(TestCase):
    def setUp(self):
        ...
        self.client_profile = Client.objects.create(
            ...
            company_name="Meeting Log Client",
            contact_email="contact@meetinglog.example",
            phone="9000000000",
            website="https://meetinglog.example",
    ...
    def test_download_finalize_style_flow_creates_log_and_clears_active_agenda(self):
        ...
        save_response = self.api_client.put(
            f"/api/meeting-agenda/clients/{self.client_profile.id}/",
            save_payload,
            format="json",
        )
        ...
        finalize_response = self.api_client.post(
            f"/api/meeting-agenda/clients/{self.client_profile.id}/finalize/",
            {"visit_date": "2026-03-29"},
            format="json",
        )
        self.assertEqual(finalize_response.status_code, status.HTTP_201_CREATED)

        self.assertEqual(MeetingAgendaLog.objects.filter(client=self.client_profile).count(), 1)
        visit_log = MeetingAgendaLog.objects.get(client=self.client_profile)
        ...
        active_agenda = MeetingAgenda.objects.get(client=self.client_profile)
        self.assertEqual(active_agenda.items.count(), 0)

        agenda_response = self.api_client.get(f"/api/meeting-agenda/clients/{self.client_profile.id}/")
    ...
    def test_logs_list_and_detail_return_saved_snapshot(self):
        MeetingAgendaLog.objects.create(
        ...
        list_response = self.api_client.get(f"/api/meeting-agenda/clients/{self.client_profile.id}/logs/")
        self.assertEqual(list_response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(list_response.data), 1)

        log_id = list_response.data[0]["id"]
        detail_response = self.api_client.get(
            f"/api/meeting-agenda/clients/{self.client_profile.id}/logs/{log_id}/"
        )
